...
* **Automatic Updates for the ShiftLeft Command Line Interface (CLI)**. The ShiftLeft CLI now automatically updates so that you don't have to reinstall the CLI whenever there are new features or fixes. Refer to [CLI Command Reference](../using-inspect-protect/using-cli/cli-reference.md) for more information.
